...
| `/iterate` warm-start command + run manifest (commands/iterate.md; new-task Phase 6 step 10) | re-paying the full lifecycle (cold Phase 0, brainstorm, plan-review, from-scratch review fan-out, per-tweak retro) to make a small delta on a just-reviewed baseline | Prop post-task-iteration-perf 2026-07; scorecard 2026-07-09-iterate + A/B 2026-07-09-iterate-cold | keep | MEASURED (iterate-cold, task 04, n=1): identical output; warm path used 3 subagents vs cold 6 and skipped ~35k tok of rediscovery + 2nd review channel + skeptic, with 0 Δ escaped defects — the cold run's extra machinery only re-derived a finding the warm delta already carried. Justified. Caveat: single doc-only run; add a repeat + a code-delta iterate task to firm the magnitude |
