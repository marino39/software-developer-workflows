...
| `/iterate` warm-start command + run manifest (commands/iterate.md; new-task Phase 6 step 10) | re-paying the full lifecycle (cold Phase 0, brainstorm, plan-review, from-scratch review fan-out, per-tweak retro) to make a small delta on a just-reviewed baseline | Prop post-task-iteration-perf 2026-07; intuition — unverified | candidate | — (needs variant + eval task 04; live scorecard owed before merge) |
