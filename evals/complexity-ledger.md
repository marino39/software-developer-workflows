...
| `/review-pr` command (commands/review-pr.md; eval task 06) | the Phase 6 review engine (multi-channel fan-out + confidence-scored consolidation + skeptic pass) could not be aimed at code you did NOT author — a teammate's PR — so inbound review fell back to a single generic pass with no skeptic filter | proposal docs/proposals/review-pr.md 2026-07-09 | keep (qualified) | Reuses Phase 6 machinery + agent contracts unchanged (adds a command surface, not engine complexity); read-only, no authoring gates. Owed: eval task 06 (frozen review fixture) scored for finding precision/recall + post-skeptic false-positive rate to evidence the engine earns its cost on foreign diffs |
