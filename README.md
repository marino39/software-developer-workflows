...
Context-compaction revision (2026-07, from `docs/proposals/2026-07-20-orchestrator-context-compaction.md` — orchestrator contexts were hitting 200–400k on the most expensive model): the orchestrator defaults to **Opus** (Fable optional at 2× per context token); artifacts pass **by path, never inlined** — the architect gained `Write` and an artifact mode (writes the design doc/plan itself, returns path + ≤200-word summary), coders get plan path + step numbers, reviewers Read the plan themselves; the run manifest is now a **live run ledger updated at every gate**, so everything binding later phases survives compaction and gate boundaries are safe `/compact` moments. Session hygiene and measurement are **automated by `install.sh`** (no manual steps): it registers a Claude Code Stop hook (`hooks/context-metrics.py`) that samples the orchestrator's context size + cache state once per turn (gate boundaries by construction; subagent sidechains excluded) into `~/.claude/metrics/context-metrics.jsonl`, sets `ENABLE_PROMPT_CACHING_1H=1` in settings env (human-gate waits under an hour re-enter cache-warm; no-op on subscription billing), and seeds a `## Compact Instructions` block in `~/.claude/CLAUDE.md` when absent, so even *automatic* compaction preserves the workflow's binding state. `hooks/context-report.sh` aggregates the samples deterministically (per-session high-water, mean, cold re-entries) and `/workflow-maintenance` check 4 surfaces the report on schedule with the decision thresholds (high-water > 250k; cold re-entries > 5 — the signal that would justify the proposal's post-GATE-3 tail hand-off). The remaining rule only you can follow: never switch the orchestrator's model or effort mid-run. Evidence attached for the change: scorecard `evals/results/2026-07-20-context-compaction-scorecard.md` (suite 98.5, 0 escaped, no regressions, orchestrator-cost column) + contract report `2026-07-20-contracts.md` (architect dual-mode 2/2); the accrued metrics now gate the remaining structural items (post-GATE-3 tail hand-off, review-lead sub-orchestrator).
