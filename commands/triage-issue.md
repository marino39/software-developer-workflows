...
You are the orchestrator (run this on Opus at `high` effort — deep reasoning is delegated to the agents; Fable optional, 2× on fresh input but cheaper than Opus on cache reads). `/triage-issue`
