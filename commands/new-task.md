...
**Pinned defaults vs. floating escalations.** The opus rung is pinned to Opus 4.8 **only where it is a frontmatter default** — `architect` and `debugger` carry the exact id `claude-opus-4-8` in `agents/<name>.md`. The Agent tool's `model` parameter accepts **aliases only** (`sonnet` | `opus` | `haiku` | `fable`; a full model id is rejected with an InputValidationError), so a per-invocation escalation lands on whatever `opus` currently resolves to, which is not necessarily 4.8. Same class of gap as the effort-override limitation below: to pin a model, edit frontmatter — the ladder cannot do it mid-run.

| Agent | Default | Escalate to | When |
|---|---|---|---|
| searcher | haiku | sonnet | empty/off-target results twice |
| researcher | sonnet | opus (alias) | contradictory sources, low-confidence twice |
| architect | opus 4.8 (pinned) | fable | Phase 1 synthesis and Phase 2 iteration-1 adversarial review are ALWAYS fable (by design, unbudgeted); otherwise same design issue unresolved 2 iterations |
| coder | sonnet | — | don't escalate model; route to debugger instead |
| reviewer | sonnet | opus (alias) → fable | same issue unresolved 2 iterations, or high-stakes diff |
| lens reviewers / consolidator / skeptics (Phase 6) | sonnet | opus (alias) → fable | same ladder and triggers as reviewer; shared fable budget |
| debugger | opus 4.8 (pinned) | fable | debugger reports unverified root cause |
